...
invalidation-cdn:
	aws --profile ${AWS_PROFILE} --region ${AWS_REGION} cloudfront create-invalidation --distribution-id ${DISTRIBUTION_ID} --paths '/*' --output text

deploy-all:
	make build
	make deploy-frontend
	make deploy-backend
	make invalidation-cdn
...
delete-backend:
	sam delete --profile ${AWS_PROFILE} --region ${AWS_REGION} --stack-name ${STACK_NAME}
